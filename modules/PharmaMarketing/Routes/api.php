...
use Modules\PharmaMarketing\Http\Controllers\Api\NotificationController;
...
use Modules\PharmaMarketing\Http\Controllers\Api\PromotionOverrideController;
...
Route::middleware('auth:sanctum')->prefix('pharma')->name('pharma.')->group(function () {
    ...
    // ── Promotion Per-Variant Overrides ────────────────────────
    Route::get('product-updates/{id}/overrides',      [PromotionOverrideController::class, 'index'])->name('product-updates.overrides.index');
    Route::put('product-updates/{id}/overrides',      [PromotionOverrideController::class, 'sync'])->name('product-updates.overrides.sync');

    // ── Notifications (delivery log + retry) ───────────────────
    // NOTE: literal 'notifications' before any parameterised routes
    Route::get('orgs/{orgId}/notifications',              [NotificationController::class, 'index'])->name('notifications.index');
    Route::get('orgs/{orgId}/notifications/{id}',         [NotificationController::class, 'show'])->name('notifications.show');
    Route::post('orgs/{orgId}/notifications/{id}/retry',  [NotificationController::class, 'retry'])->name('notifications.retry');
